...
@section('title', 'FoodAtlas - List of Collections')
  ...
  <main class="py-16 lg:py-24">
    <div class="container mx-auto px-4 lg:px-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach ($collections as $collection)
          ...
          <figure class="aspect-[4/3] overflow-hidden rounded-t-2xl">
            <img src="{{ $collection->image }}" alt="{{ $collection->title }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
          </figure>
      ...
      <div class="flex justify-center mt-12">
        <div class="join">
          <a href="{{ route('collections.index', ['page' => $collections->currentPage() - 1]) }}" class="join-item btn btn-outline btn-sm">«</a>
          @for ($i = 1; $i <= $collections->total()/3; $i++)
            <a href="{{ route('collections.index', ['page' => $i]) }}" class="join-item btn btn-primary btn-sm">{{ $i }}</a>
          @endfor
          <a href="{{ route('collections.index', ['page' => $collections->currentPage() + 1]) }}" class="join-item btn btn-outline btn-sm">»</a>
        </div>
      </div>
