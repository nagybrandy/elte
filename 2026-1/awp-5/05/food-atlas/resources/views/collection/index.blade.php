...
@section('title', 'Food Atlas - Collections')
      ...
      @if($collections->isEmpty())
        <div class="card bg-base-200 border border-base-300 shadow-md">
          <div class="card-body items-center text-center py-16">
            ...
            <h2 class="card-title font-serif text-xl text-base-content/80">No collections yet</h2>
            <p class="text-base-content/60 max-w-md">Collections you add or import will appear here.</p>
          </div>
        </div>
      @else
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          @foreach ($collections as $collection)
          <a href="{{ route('collections.show', $collection) }}" class="card bg-base-200 shadow-lg hover:shadow-2xl transition-all duration-300 border border-base-300 hover:border-primary/30 group focus:outline-none focus:ring-2 focus:ring-primary/50 rounded-2xl">
