<div class="navbar bg-white/90 backdrop-blur-lg sticky top-0 z-50 border-b border-base-300 shadow-sm">
    <div class="navbar-start">
      <a href={{route('landing')}} class="btn btn-ghost text-xl">
        <img src="/image.png" alt="Food Atlas logo" class="h-10" />
      </a>
    </div>
    <div class="navbar-center hidden lg:flex">
      <ul class="menu menu-horizontal px-1 gap-6">
        <li><a href={{route('landing')}} class="text-sm font-medium hover:text-primary transition-colors">Home</a></li>
        <li><a href={{route('recipes.index')}} class="text-sm font-medium hover:text-primary transition-colors">Recipes</a></li>
        <li><a href={{route('collections.index')}} class="text-sm font-medium hover:text-primary transition-colors">Collections</a></li>
      </ul>
    </div>
    <div class="navbar-end gap-2">
      ...
      @if(auth()->check())
        <span class="text-sm font-medium text-base-content/70">{{ Auth::user()->name }}</span>
        <a href={{route('dashboard')}} class="btn btn-primary btn-sm hidden lg:flex hover:bg-base-200">Dashboard</a>
        <form method="POST" action={{route('logout')}}>
          @csrf
          <button type="submit" class="btn btn-outline btn-sm hidden lg:flex hover:bg-base-200">Log out</button>
        </form>
      ...
      <div class="dropdown dropdown-end lg:hidden">
        ...
        <ul tabindex="0" class="menu menu-sm dropdown-content mt-3 z-[1] p-3 shadow-xl bg-white rounded-box w-56 border border-base-300">
          <li><a href={{route('landing')}} class="hover:bg-base-200">Home</a></li>
          ...
          @if(auth()->check())
            <li><span class="text-sm font-medium text-base-content/70">{{ Auth::user()->name }}</span></li>
            <li><a href={{route('dashboard')}} class="hover:bg-base-200">Dashboard</a></li>
            <form method="POST" action={{route('logout')}}>
              @csrf
              <button type="submit" class="btn btn-outline btn-sm w-full">Log out</button>
            </form>
          @else
