...
<main class="py-16 lg:py-24">
    <div class="container mx-auto px-4 lg:px-8 max-w-4xl">
      ...
      <!-- FORM -->
      <form class="space-y-8" action="{{ isset($recipe) && $recipe->id ? route('recipes.update', $recipe->id) : route('recipes.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="card bg-base-200 border border-base-300 shadow-lg">
          <div class="card-body">
            ...
            <div class="form-control w-full mb-4">
              ...
              <input type="text" id="title" name="title" value="{{ $recipe->title ?? '' }}" placeholder="e.g., Creamy Carbonara Pasta" class="input input-bordered w-full" required />
            </div>
            <div class="form-control w-full mb-4">
            ...
            <div class="form-control w-full mb-4">
              ...
              <input type="text" id="recipe-image" name="image" value="{{ $recipe->image ?? old('image') }}" placeholder="https://example.com/image.jpg" class="input input-bordered w-full" />
              <label class="label">
                <span class="label-text-alt">Or upload an image file</span>
              </label>
              @if($recipe->image_file)
                <img src="{{ asset('storage/' . $recipe->image_file) }}" alt="Recipe Image" class="w-full h-auto" />
              @endif
